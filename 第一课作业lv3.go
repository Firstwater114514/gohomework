...
way3:
	...
	for {
		fmt.Printf("敌方血量：%d/%d 疾风值：%d 攻击力：%d 护甲值：%d\n", hp_, hpm_, jf, atk_, def_)
		fmt.Printf("你的血量：%d/%d 魔法值：%d/%d 攻击力：%d 护甲值：%d\n", hp, hpm, pp, ppm, atk, def)
		fmt.Printf("选择你的战斗方式:\n1.普通攻击   2.使用技能(cd:%d/100)（耗蓝125,加10攻击力）   3.购买护甲（70）   4.使用血瓶（有%d个）\n5.使用蓝瓶（有%d个）   6.购买血瓶（120）   7.购买蓝瓶（60）   8.加强武器（65）\n114514.投降逃跑                              你的经济：%d$\n", cd, xp, lp, money)
		var op2 int32
		fmt.Scanf("%d", &op2)
	way4:
		fmt.Scanf("%d", &op2)
		...
		case 2:
			if pp >= 125 && cd >= 100 {
				fmt.Printf("你使用了技能，攻击力+10，蓝量-125\n")
				cd -= 100
				money += 20
				atk = atk + 10
				pp = pp - 125
				if hp_-atk-35+def_ > hp_ {
					hp_ = hp_ + 0
					fmt.Printf("敌方受到了0点攻击\n")
				} else {
					hp_ = hp_ - atk - 35 + def_
		...
		case 8:
			if money >= 65 {
				money -= 65
				atk += 15
				fmt.Println("攻击力提升15点！")
			} else {
				fmt.Println("钱包：看我干嘛？我又没钱")
			}
		case 114514:
			fmt.Printf("你给路达油~~\n你逃跑力\n")
			return
		default:
			fmt.Println("你在按什么啊？团长！")
			goto way4
		}
		if cd < 100 {
			cd += 20
		...
		if hp_ <= hpm_/2 {
			if xp_ >= 1 {
				...
				fmt.Println("敌方使用血瓶，血量+300")
			} else if money >= 120 && xp_ < 2 {
				money_ -= 120
				xp_++
				fmt.Printf("敌方已做出行动\n")
			} else if jf >= 100 {
				...
				if hp-atk_-50+def > hp {
					hp = hp + 0
					fmt.Printf("你受到了0点的伤害\n")
				} else {
					hp = hp - atk_ - 50 + def
					fmt.Printf("你受到了%d点的伤害\n", atk_+50-def)
				}
				money += 10
				if hp <= 0 {
					fmt.Printf("鉴定为“只因”\n你寄力,被撅力（大悲）\n")
